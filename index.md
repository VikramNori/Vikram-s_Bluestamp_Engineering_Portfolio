...
while True:
    ...
    time.sleep(0.5)
```
I was originally going to combine the 2 codes and make a tracker that tracked both the ISS and CSS at the same time, however, that would require 2 pyportal objects to be active at the same time, which is impossible to do in circuitpython. I decided to settle on the ISS or CSS tracker. How it works is that it gives you an if loop. It asks you to put in ISS for the ISS's position or CSS for the CSS's position, and displays the latitude and longitude of the satellite currently being tracked. I just put the code for both trackers on top of each other.
```python
import time
import math
import board
...
which_one = input("Would you like the location of the ISS or the CSS? \nType 'ISS' for ISS and 'CSS' for CSS. \n -->")
...
if "ISS" in which_one:
    ...
    DATA_SOURCE = "http://api.open-notify.org/iss-now.json"
    DATA_LOCATION = ["iss_position"]
    print (f"Retrieving data from {DATA_SOURCE}...")
    WIDTH = board.DISPLAY.width
    HEIGHT = board.DISPLAY.height
    ...
    pyportal.get_local_time()
    if AttributeError:
        print("Could not connect to the internet. Try again.")
    print("Tracking the CSS...")
    lat_label = Label(FONT, text="Lat: ---", color=0xFFFFFF, x=10, y=40)
    lon_label = Label(FONT, text="Lon: ---", color=0xFFFFFF, x=10, y=70)
    board.DISPLAY.auto_refresh = True
    pyportal.splash.append(lat_label)
    ...
    def update_display(current_time, update_iss=False):
        ...
        if update_iss:
            ...
            if result and len(result) == 4:
                ...
                # Safe coordinate formatting
                if isinstance(lat, float) and isinstance(lon, float):
                    lat_label.text = "CSS_Lat: {:.2f}".format(lat)
                    lon_label.text = "CSS_Lon: {:.2f}".format(lon)
